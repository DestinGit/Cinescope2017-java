...
*/
        // Par défaut la jComboBoxArrondissement n'est pas selectionnable
        jComboBoxArrondissement.setEnabled(false);

        this.jTextFieldCode.setText("23");
        this.jTextFieldNom.setText("Odéon");
        this.jTextFieldAdresse.setText("Rue de la paix");
    ...
    private void jButtonValiderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonValiderActionPerformed
        ...
        if (lbOK) {
            try {
                ...
                lpst.setInt(2, (int) resultatArrondissement.getIdArrondissement());
                ...
                int r = lpst.executeUpdate();
                if (r == 1) {
                    ResultSet lrsPK = lpst.getGeneratedKeys();
                    if (lrsPK.next()) {
                        System.out.println("Nouvelle clé : " + lrsPK.getInt(1));
                        int idCinema = lrsPK.getInt(1);
                        PreparedStatement lpstSalles = icnx.prepareStatement("INSERT INTO salle(id_cinema, nom_salle, description_salle) VALUES(?,?,?)");
                        int liLigne;
                        liLigne = this.jTableSalle.getRowCount();
                        for (int i = 0; i < liLigne; i++) {
                            lpstSalles.setInt(1, idCinema);
                            lpstSalles.setString(2, jTableSalle.getValueAt(i, 1).toString());
                            lpstSalles.setString(3, jTableSalle.getValueAt(i, 2).toString());
                            r = lpstSalles.executeUpdate();
